...
def delq_table(delq, values) -> pd.DataFrame:
    ...
    ** Private Function **
    ...
    '''
    columns = list(delq)
    bins = np.array([ 0., 30, 60., 90., np.inf])
...
def plot_rollrate(delq, mobs, values=None, ax=None, colors=None, 
    ...
    labels_ = ["Clean (0)", "1-30", "31-60", "61-90", "90+"]
...
def vintage_table(X, groupby, dpd_cols, dpd_geq=30, 
                  func=np.nanmean, values=None, factor=1):
    '''Vintage analysis'''
    ...
    a = np.where(np.isnan(a), np.nan, a>=1)
    
    if values is not None: 
        a *= X[values].values.copy() * factor
    
    new_X[dpd_cols] = a
    new_X["N"] = 1
    aggfnc = {**{"N":"count"},
              **dict([(c, func) for c in dpd_cols])}
...
def dpd_table(X, groupby, dpd_col, os_val=False, applyformat=True):
    
    '''DPD binning'''
    groupby = groupby if isinstance(groupby, list) else [groupby]
    new_X = X[groupby].copy()
    
    if os_val: 
        col0, col1 = "M00_OS", f"{dpd_col}_OS"
        new_X[col0] = X[col0]/10**6
        new_X[col1] = X[col1]/10**6
    else: 
        col0, col1 = "M00", dpd_col
        new_X[col0] = 1
        new_X[col1] = X[col1].notna().astype(int)
    
    # Binning
    bins = np.array([-np.inf, 0., 30, 60., 90., np.inf])
    states = ["(1) x=0", "(2) 0<x<=30" , "(3) 30<x<=60", 
              "(4) 60<x<=90", "(5) x>90"]
    
    # right=True ==> bins[i-1] < x <= bins[i]
    delq = X[dpd_col].values.ravel().copy()
    delq = np.digitize(delq, bins, right=True)
    for n,state in enumerate(states,1):
        new_X[state] = np.where(delq==n, new_X[col1], np.nan)
        
    # Aggregate function and grouping
    aggfnc = {col0:"sum", col1:np.nansum}
    aggfnc.update(dict([(s,np.nansum) for s in states]))
    new_X = new_X.groupby(groupby).agg(aggfnc)
    index = new_X.index

    # Percentage (%)
    factor = 100/np.fmax(new_X[[col1]].values, 1)
    percent = new_X[states].values * factor
    
    # Contruct table
    a = [new_X[[col0, col1]].values]
    for n in range(len(states)):
        a += [np.hstack((new_X.values[:,[n+2]],
                         percent[:,[n]]))]

    # Rename columns
    columns  = [("",col0), ("",col1)] 
    columns += [(s0,s1) for s0 in states 
                for s1 in ["MB" if os_val 
                           else "Count","(%)"]]
    
    # Apply format
    a = pd.DataFrame(np.hstack(a))
    if applyformat:
        for n,col in enumerate(a.columns):
            if n not in np.arange(3,12,2):
                if os_val: a[col] = a[col].map('{:,.1f}'.format)
                else: a[col] = (a[col].fillna(0).astype(int)
                                .map('{:,d}'.format))
            else: a[col] = a[col].map('{:.1f}'.format)
    a.columns = pd.MultiIndex.from_tuples(columns)
    return a.set_index(new_X.index)

def pivot_table(X, index, columns, mob=2, state=1, 
                return_counts=True, decimal=1, applyformat=True):
    
    # Initialize paramters
    states = {0: ([-np.inf, 0], "x=0"), 
              1: ([ 0, 30], "0<x<=30"),
              2: ([30, 60], "30<x<=60"), 
              3: ([60, 90], "60<x<=90"),
              4: ([90, np.inf], "x>90")}

    col = "M{}".format(str(mob).zfill(2))
    bal = "M{}_OS".format(str(mob).zfill(2))
    bins, state = states[state]

    new_X = X.loc[:,index + columns + [col, bal]].copy()
    new_X[bal] = new_X[bal]/10**6
    
    # right=True ==> bins[i-1] < x <= bins[i]
    delq = new_X[col].values.ravel()
    cond = np.digitize(delq, bins, right=True)==1
    n_samples = np.nansum(cond)
    new_X[col] = 1
    
    # Pivot table
    if n_samples>0:
        values = [col if return_counts else bal]
        kwargs = dict(values=values, index=index, 
                      columns=columns, aggfunc=np.nansum, 
                      margins=True, margins_name="Total")
        a = pd.pivot_table(new_X[cond], **kwargs)

        # Create new columns
        name = ["Count" if return_counts else "O/S (MB)"]
        ...
        if applyformat:
            ...
            for n in range(a.shape[1]):
                a.iloc[:,n] = a.iloc[:,n].fillna(0).map(num)
        return a
    else: return None